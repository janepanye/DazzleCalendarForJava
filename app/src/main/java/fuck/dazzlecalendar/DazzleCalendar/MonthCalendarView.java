 ...
 */
//月视图控件，里面添加42个子view，并计算位置
public class MonthCalendarView extends ViewGroup implements View.OnClickListener {
    //当前月对应的种子时间
    public Calendar monthCalendar = Calendar.getInstance();
    //监听事件
    ...
    protected void onLayout(boolean b, int left, int top, int right, int bottom) {
        ...
        for (int row = 0;row < 6;row ++)//行
            for (int col = 0;col < 7;col ++) {//列
                //得到第几个view
                View childView = getChildAt(row * 7 + col);
                //除了位置之外
                childView.layout(col * itemWidth, row * itemHeight, col * itemWidth + itemWidth, row * itemHeight
                        + itemHeight);
                childView.setTag(row * 7 + col);
                //还要设置宽高
    ...
    public void reloadCurrMonth() {
        ArrayList<Calendar> currMonthCalendars = CalendarTool.getMonthCalendars(monthCalendar);
        for (int index = 0;index < 42;index ++) {
            CalendarDayView calendarDayView = (CalendarDayView)getChildAt(index);
            calendarDayView.calendarType = 1;
            calendarDayView.dayCalendar = currMonthCalendars.get(index);
            calendarDayView.weekMonthCalendar = monthCalendar;
